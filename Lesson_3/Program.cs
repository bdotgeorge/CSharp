...
namespace Program
{
    class Lesson_3
    {
        ...
        public static bool validList(int length){
            if((length % 6) != 0 || length < 4)
                return false;
            return true;
        }
        struct Coordinate{
        ...
        static void TaskTwentyOne(){
            Console.WriteLine("A program that takes as input the coordinates of two points and finds the distance between them in 3D space");
        ...
        static void TaskNineteenPalidrom(){
                Console.WriteLine("Enter an integer to test for a palindrome");
                string p = Console.ReadLine()!;
                Palidrom(stringToInt(ref p));
        }

        static void TaskTwentyThreeCoub(){
            Console.WriteLine("A program that takes a number (N) as input and produces a table of cubes of numbers from 1 to N");
            string p = Console.ReadLine()!;
